## CV
# [Shamkolovich Sergey](https://bipbap.ru/wp-content/uploads/2020/04/Risunki-dlya-srisovki-dlya-malchikov-i-parnej-samye-prikolnye-i-krutye-16-1.jpg)
# Contacts 
<!--Списки-->
* Adress: st. O.Koshevogo; Minsk, Belarus;
* E-mail: shamkolovich95@mail.ru;
* Github: [shamkolovich95](github.com/shamkolovich95).
# Summary
I just started learning how to program, but I hope to finish this course and continue my career in this direction.
# Skills
As I said, I’m at the beginning of the road.
# Code examples
```
<html>
  <head>
    <title> Гостевая Head First </title>
  </head>
  <body>
    <h1>Добро пожаловать в новую и усовершенствованную гостевую Head First </h1>
    <img src="images/drinks.gif">
    <p>
       Заходите к нам каждый вечер, чтобы попробовать освежающие
       <a href="beverages/elixir.html"> напитки </a>, поболтать и возможно
       <em>станцевать разок другой</em>.Всегда обеспечен беспроводной
       доступ (захватите с собой свой ноутбук).
    </p>
    <h2>Directions</h2>
    <p>
      Вы найдете нас в центре Webville.   
      яесли вам нужна помощь, чтобы найти нас, используйте 
      наши <a href="about/directions.html"> указатели </a>.
    </p>
  </body>
</html>
```
# Education
<!--списки-->
* Belarusian State Technological University.
  * Faculty of chemical technology and engineering.
* Minsk State Polytechnic College.
  * Installation and operation of electrical equipment.
# English
A2 (Pre-intermediate).